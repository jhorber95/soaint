...
export class ProductoUpdatePage {
  ...
  precionInput = element(by.id('field_precion'));
  ...
  async setPrecionInput(precion: string): Promise<void> {
    await this.precionInput.sendKeys(precion);
  }

  async getPrecionInput(): Promise<string> {
    return await this.precionInput.getAttribute('value');
  }
